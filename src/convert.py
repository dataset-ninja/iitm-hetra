...
def create_ann(image_path):
    labels = []
    if os.path.basename(os.path.dirname(image_path)) == "images":
        ann_path = os.path.join(
            os.path.dirname(os.path.dirname(image_path)), "xmls", (sly.fs.get_file_name(image_path) + ".xml")
        )
    else:
        ann_path = os.path.join(
            os.path.dirname(os.path.dirname(image_path)), "xmls_test", (sly.fs.get_file_name(image_path) + ".xml")
        )
...
ann_pathes = glob("/mnt/c/users/german/documents/IITM-HeTra_v2/Dataset-1/xmls/*.xml")
oc_dict = {}
for ann in ann_pathes:
    with open(ann, "r") as f:
        ann_xml_content = f.read()
...
def convert_and_upload_supervisely_project(
    ...
    dataset_path = "/mnt/c/users/german/documents/IITM-HeTra_v2/Dataset-1"
    dataset_names = {
        "trainval": "images",
        "test": "images_test",
    }
    ...
    for ds, path in dataset_names.items():
        dataset = api.dataset.create(project_id=project.id, name=ds, change_name_if_conflict=True)
        images_pathes = [
            filename
